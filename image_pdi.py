...
class Image:
    ...
    def __init__(self, filename):
        ...
        self.kernel = np.ones((6, 6), np.float32) / \
            25  # definição do KERNEL/MÁSCARA
    ...
    def fatiamento(self, plane):
        ...
        p = np.array([[int(np.binary_repr(a[i][j], 8)[8 - plane]) *
                       255 for j in range(0, a.shape[1])] for i in range(0, a.shape[0])])
